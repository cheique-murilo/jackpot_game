...
# Função para enviar email ao administrador se o jogador ganhar
def send_email_winner(email):
    smtp_server = "smtp.gmail.com"
    smtp_port = 587
    sender_email = "seuemail@gmail.com"  # Substitua pelo seu email do Gmail
    sender_password = "sua_senha_de_aplicativo"  # Substitua pela senha de aplicativo gerada
    receiver_email = "bolaoanimal@gmail.com"

    subject = "🎉 Jackpot Ganhador!"
    body = f"O jogador {email} ganhou o jackpot em {datetime.datetime.now().strftime('%d/%m/%Y %H:%M:%S')}!"

    message = f"Subject: {subject}\n\n{body}"

    print(f"Tentando enviar email para {receiver_email} sobre o ganhador {email}...")
...
def play(request: PlayRequest, db: Session = Depends(get_db)):
    ...
    if win:
        ...
        return {"message": "🎉 Parabéns! Você ganhou!", "numbers": numbers, "win": True, "attemptsLeft": player.attempts}
